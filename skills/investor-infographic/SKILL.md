...
- For existing company hand-drawn pairs, do not leave financial-dashboard blocks in `经营之道`. The visible body of `经营之道` should not contain repeated full-dashboard items such as `全年营收`, `净利润`, `自由现金流`, `最新市值`, `股价/市值`, `PE`, `ROI/ROIC`, or `2026 CapEx`; those belong in `经营全景`. `经营之道` may use selective non-financial or operating evidence only when it explains the moat or flywheel, such as active users, installed base, developer ecosystem, production volume, patents, network scale, or a specific strategic milestone.
...
- Financial duplication audit: scan or visually inspect `经营之道` separately from `经营全景`. Reject if `经营之道` still has full financial-dashboard blocks for revenue, net income, free cash flow, market cap, PE, ROI/ROIC, or CapEx. These metrics should appear in `经营全景`; `经营之道` should use only logic/evidence panels.
